...
class TestSeg(unittest.TestCase):
    ...
    def load_labelmap(self) -> Dict[Tuple, int]:
        ...
        for idx, line in enumerate(lines):
            ...
            palatte_dict[(r, g, b)] = idx
        return palatte_dict

    def test_image_convert(self):
        gt_img_dict = dict()
        for key in ['color', 'instanceIds', 'labelIds', 'labelTrainIds.png']:
            gt_img_dict[key] = Image.open(osp.join('tests/seg_images', f'munster_000000_000019_gtFine_{key}.png'))

        palatte_dict = self.load_labelmap()
        gt_img_dict['ymir'] = convert_rgb_to_label_id(gt_img_dict['color'], palatte_dict)
